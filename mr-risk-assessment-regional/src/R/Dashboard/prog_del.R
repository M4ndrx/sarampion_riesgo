##############################################################
# Herramienta digital Análisis de Riesgo SR - prog_del.R
...
rend_title_map <- function(LANG_TLS,COUNTRY_NAME,YEAR_LIST,admin1,var) {
  YEAR_1=YEAR_LIST[1];YEAR_2=YEAR_LIST[2];YEAR_3=YEAR_LIST[3];YEAR_4=YEAR_LIST[4];YEAR_5=YEAR_LIST[5];
  var_text <- case_when(
    var == "TOTAL_PR" ~ paste0(lang_label_tls(LANG_TLS,"prog_title_map_total_pr")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "tendencia_SRP1" ~ paste0(lang_label_tls(LANG_TLS,"prog_title_map_trend_mmr1")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "tendencia_SRP2" ~ paste0(lang_label_tls(LANG_TLS,"prog_title_map_trend_mmr2")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_1," - ",YEAR_5,")"),
    var == "tasa_des_srp1_srp2" ~ paste0(lang_label_tls(LANG_TLS,"prog_title_map_dropout_1")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_5,")"),
    var == "tasa_des_penta1_srp1" ~ paste0(lang_label_tls(LANG_TLS,"prog_title_map_dropout_2")," ",admin1_transform(LANG_TLS,COUNTRY_NAME,admin1)," (",YEAR_5,")")
  )
  return(var_text)
}


rend_plot_map_data <- function(LANG_TLS,COUNTRY_NAME,YEAR_LIST,ZERO_POB_LIST,CUT_OFFS,map_data,data,var_to_summarise,admin1,admin1_id,admin1_geo_id_df) {
  ...
  if (var_to_summarise == "TOTAL_PR") {
    ...
    map <- leaflet(map_data,options = leafletOptions(doubleClickZoom = T, attributionControl = F, zoomSnap=0.1, zoomDelta=0.1)) %>%
      ...
      addLegend(layerId = "map_title","topright",color = "white", opacity = 0,labels=HTML(paste0("<strong>",rend_title_map(LANG_TLS,COUNTRY_NAME,YEAR_LIST,admin1,var_to_summarise),"</strong>"))) %>%
  ...
  if (var_to_summarise %in% c("tendencia_SRP1","tendencia_SRP2")) {
    ...
    map <- leaflet(map_data,options = leafletOptions(doubleClickZoom = T, attributionControl = F, zoomSnap=0.1, zoomDelta=0.1)) %>%
      ...
      addLegend(layerId = "map_title","topright",color = "white", opacity = 0,labels=HTML(paste0("<strong>",rend_title_map(LANG_TLS,COUNTRY_NAME,YEAR_LIST,admin1,var_to_summarise),"</strong>"))) %>%
      addLegend(title = lang_label_tls(LANG_TLS,"trend"),colors = legend_colors,labels = legend_values, opacity = 0.5, position = 'topright')
  }
    
  if (var_to_summarise %in% c("tasa_des_srp1_srp2","tasa_des_penta1_srp1")) {
    map_data$var <- round(map_data$var,1)
    ...
    shape_label <- sprintf("<strong>%s</strong>, %s<br/>%s: %s",
                           map_data$ADMIN2,
                           map_data$ADMIN1,
                           lang_label_tls(LANG_TLS,"dropout"),
                           map_data$var
    ) %>% lapply(HTML)
    ...
    map <- leaflet(map_data,options = leafletOptions(doubleClickZoom = T, attributionControl = F, zoomSnap=0.1, zoomDelta=0.1)) %>%
      ...
      addLegend(layerId = "map_title","topright",color = "white", opacity = 0,labels=HTML(paste0("<strong>",rend_title_map(LANG_TLS,COUNTRY_NAME,YEAR_LIST,admin1,var_to_summarise),"</strong>"))) %>%
      addLegend(title = lang_label_tls(LANG_TLS,"dropout"),colors = legend_colors,labels = legend_values, opacity = 0.5, position = 'topright')
  }
...
rend_get_data_table <- function(LANG_TLS,CUT_OFFS,data,admin1_id) {
  
  data$risk_level <- get_risk_level(LANG_TLS,CUT_OFFS,"PROG_DEL",data$TOTAL_PR)
  
  data <- data %>% select(`ADMIN1 GEO_ID`,ADMIN1,ADMIN2,TOTAL_PR,risk_level,
                          tendencia_SRP1,tendencia_SRP1_PR,
                          tendencia_SRP2,tendencia_SRP2_PR,
                          d_SRP1,d_SRP2,d_PENTA1,
                          tasa_des_srp1_srp2,tasa_des_srp1_srp2_PR,
                          tasa_des_penta1_srp1,tasa_des_penta1_srp1_PR) %>%
    mutate(
      d_SRP1=cFormat(d_SRP1,0),
      d_SRP2=cFormat(d_SRP2,0),
      d_PENTA1=cFormat(d_PENTA1,0),
      tendencia_SRP1 = round((tendencia_SRP1),1),
      tendencia_SRP2 = round((tendencia_SRP2),1),
      tasa_des_srp1_srp2 = round((tasa_des_srp1_srp2),1),
      tasa_des_penta1_srp1 = round((tasa_des_penta1_srp1),1)
    )
  ...
  if (admin1_id == 0) {
    ...
    colnames(data) <- c(lang_label_tls(LANG_TLS,"table_admin1_name"),lang_label_tls(LANG_TLS,"table_admin2_name"),lang_label_tls(LANG_TLS,"total_pr"),lang_label_tls(LANG_TLS,"risk_level"),
                        lang_label_tls(LANG_TLS,"prog_table_trend_mmr1"),lang_label_tls(LANG_TLS,"prog_table_trend_mmr1_pr"),
                        lang_label_tls(LANG_TLS,"prog_table_trend_mmr2"),lang_label_tls(LANG_TLS,"prog_table_trend_mmr2_pr"),
                        lang_label_tls(LANG_TLS,"prog_table_doses_mmr1"),
                        lang_label_tls(LANG_TLS,"prog_table_doses_mmr2"),
                        lang_label_tls(LANG_TLS,"prog_table_doses_penta1"),
                        lang_label_tls(LANG_TLS,"prog_title_map_dropout_1"),lang_label_tls(LANG_TLS,"prog_table_dropout_1_pr"),
                        lang_label_tls(LANG_TLS,"prog_title_map_dropout_2"),lang_label_tls(LANG_TLS,"prog_table_dropout_2_pr")
    )
  } else {
    ...
    datatable(
      ...
      options = list(
        ...
        buttons = list(
          ...
          list(extend='csv',filename=paste(lang_label_tls(LANG_TLS,"data"),lang_label_tls(LANG_TLS,"PROG_DEL"),admin1_id)),
          list(extend='excel', filename=paste(lang_label_tls(LANG_TLS,"data"),lang_label_tls(LANG_TLS,"PROG_DEL"),admin1_id))
        ),
        class = "display"
      )
    ) %>% formatStyle(
      ...
      c(lang_label_tls(LANG_TLS,"total_pr"),
        lang_label_tls(LANG_TLS,"prog_table_trend_mmr1_pr"),
        lang_label_tls(LANG_TLS,"prog_table_trend_mmr2_pr"),
        lang_label_tls(LANG_TLS,"prog_table_dropout_1_pr"),
        lang_label_tls(LANG_TLS,"prog_table_dropout_2_pr")),
